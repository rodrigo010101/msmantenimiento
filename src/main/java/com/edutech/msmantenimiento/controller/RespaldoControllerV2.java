...
public class RespaldoControllerV2 {
    ...
    public ResponseEntity<EntityModel<Respaldo>> createRespaldo(@RequestBody Respaldo respaldo) {
        ...
        return ResponseEntity
                .created(linkTo(methodOn(RespaldoControllerV2.class).getRespaldoById(newRespaldo.getIdrespaldo()))
                        .toUri())
                .body(assembler.toModel(newRespaldo));
    }

    @PutMapping(value = "/{id}", produces = MediaTypes.HAL_JSON_VALUE)
    public ResponseEntity<EntityModel<Respaldo>> updateRespaldo(@PathVariable int codigo,
            @RequestBody Respaldo respaldo) {
        respaldo.setIdrespaldo(codigo);
        Respaldo updateRespaldo = respaldoService.save(respaldo);
        return ResponseEntity
                .ok(assembler.toModel(updateRespaldo));
    }
